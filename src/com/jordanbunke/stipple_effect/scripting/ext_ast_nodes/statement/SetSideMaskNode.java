...
public final class SetSideMaskNode extends SEExtStatementNode {
    ...
    public FuncControlFlow execute(final SymbolTable symbolTable) {
        final int[] sideMask = ((ScriptArray) getValues(symbolTable)[0])
                .stream().mapToInt(s -> (int) s).toArray();

        if (ScriptSelectionUtils.invalidSideMask(sideMask, getArgs()[0]))
